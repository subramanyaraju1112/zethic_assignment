...
function App() {
  ...
  const getPieData = () => {
    const filteredByAge = USERS.filter(
      (user) => user.age <= age && user.age > age - 5
    );
    const filterByCarMaker = filteredByAge?.filter(
      (user) => user.vehicle.manufacturer === maker
    );
    let temp = [];
    ...
    temp.forEach((vehicle) => {
      if (!vehicleAgeFrequency[vehicle.carAge])
    ...
    Object.keys(vehicleAgeFrequency).forEach((vehicle) => {
      vehicleAgeData.push({
        name: vehicle,
        value: vehicleAgeFrequency[vehicle],
      });
    });
  ...
  console.log(filteredByCars, "::stattt");
      ...
      <div className="App" style={{ display: "flex" }}>
        ...
        >
          ...
          <List
            ...
            rowRenderer={({ index, style, parent }) => {
              ...
              return (
                <div key={user.userId} style={style}>
                  <h5
                    className="mont"
                    onClick={() => {
        ...
        >
          ...
          <div className="input-2">
            ...
            {Array.from(manufacturerSet).length > 0 && (
              <select
                name="maker"
                value={maker}
                onChange={(e) => {
          ...
          <button style={{ margin: "15px" }} onClick={getPieData}>
            Show Demographics
          </button>
          {vehicleValues && (
            <div
          ...
          <div className="pagination">
                ...
                {slicedVehicles.map((vehicle) => {
                  return (
                    <h6
                      ...
                      onClick={() => {
                        console.log(vehicle, ":: Filtered by car");
                        let filteredCarList = USERS.filter(
                          (user) =>
                        ...
                        console.log(filteredCarList, ":: Filter CAR");
                        setfilteredByCars(filteredCarList);
                      }}
                    >
                      {vehicle}
                    </h6>
            ...
            {/* Right sidebars based on user click funtionality */}

            {filteredByCars.length > 0 && (
              <RightSidebar
                open={filteredByCars.length > 0}
                onClose={() => setfilteredByCars([])}
                title="List of Users with car"
              >
                {filteredByCars.map(({ name }, index) => (
                  <h5 key={index}>{`${index + 1}. ${name}`}</h5>
                ))}
              </RightSidebar>
            )}

            {showUserDetails && Object.keys(userDetails).length > 0 && (
              <RightSidebar
              ...
              >
                <div style={{ height: "95%", padding: "10px" }}>
                  ...
                  <h6>
                    Vehicle:{" "}
                    {userDetails.vehicle.manufacturer +
                      " " +
                      userDetails.vehicle.model}
                  </h6>
                </div>
              </RightSidebar>
            )}
            ...
            <div style={{ display: "flex" }}>
              <span
                style={{ margin: "5px", cursor: "pointer", color: "#00ffff" }}
                onClick={() => {
              ...
              <span style={{ margin: "5px", color: "#00ffff" }}>
                {currentPage}
              </span>
              <span
                style={{ margin: "5px", cursor: "pointer", color: "#00ffff" }}
